...
void allocate_memory(list_t * freelist, list_t * alloclist, int pid, int blocksize, int policy) {
    ...
    */
    ...
    while (current != NULL && !found){
      blk = current->blk;
      found = (blk->end - blk->start + 1) >= blocksize;
      
      if (found) {
        list_remove_at_index(freelist, list_get_index_of(freelist, blk));
        blk->pid = pid;
        blk->end = blk->start + blocksize - 1;

        list_add_ascending_by_address(alloclist, blk);

        if ((blk->end - blk->start + 1) < (blocksize - 1)){
          fragment = (block_t *)malloc(sizeof(block_t));
          fragment->pid = 0;
          fragment->start = blk->end + 1;
          fragment->end = blk->end + (blocksize - (blk->end - blk->start + 1));
...
void deallocate_memory(list_t * alloclist, list_t * freelist, int pid, int policy) { 
    ...
    */
    ...
    if (index != -1) {
      block_t *deallocate_block = list_remove_at_index(alloclist, index);
      deallocate_block->pid = 0;

      if (policy == 1) {
        list_add_to_back(freelist,deallocate_block);
      } else if (policy == 2) {
        list_add_ascending_by_blocksize(freelist, deallocate_block);
      } else if (policy == 3) {
        list_add_descending_by_blocksize(freelist, deallocate_block);
      } else {
          printf("Error: Invalid policy\n");
      }
    } else {
      printf("Error: Cannot locate Memory Used by PID: %d\n", pid);
    }
}
