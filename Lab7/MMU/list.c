...
void list_add_ascending_by_address(list_t *l, block_t *newblk){
   ...
   */
   ...
   if(l->head == NULL || newblk->start <= l->head->blk->start) {
    ...
    while (current->next != NULL && newblk->start > current->next->blk->start) {
      current = current->next;
    }
...
void list_add_ascending_by_blocksize(list_t *l, block_t *newblk){
   ...
   */
   ...
   if (l->head == NULL || compareSize(newblk_size, l->head->blk)){
    newNode->next = l->head;
    l->head = newNode;
   } else {
    node_t *current = l->head;
    while (current->next != NULL && !compareSize(newblk_size, current->next->blk)) {
      current = current->next;
    }
    newNode->next = current->next;
    current->next = newNode;
...
void list_coalese_nodes(list_t *l){ 
   ...
   */
   ...
   while (current != NULL && current->next != NULL) {
    ...
    int next_start = current->next->blk->start;
...
bool list_is_in_by_pid(list_t *l, int pid){ 
  ...
  */
  ...
  while(current != NULL){
    // Compares both values to see if they are the same
