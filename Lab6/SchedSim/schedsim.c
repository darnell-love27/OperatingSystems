...
{ 
   ...
   while (complete != n) {
    for (int j = 0; j < n; j++) {
      if (plist[j].art <= t && plist[j].bt < minm && plist[j].bt > 0) {
        minm = plist[j].bt;
        shortest = j;
        flag = 1;
      }
  ...
  // Decrementing remainning time of selected process
   plist[shortest].bt--;
   minm = plist[shortest].bt;
...
{ 
    ...
    const ProcessType *p2 = (const ProcessType *)this;
